...
const metricsToSvg = async (lighthouseMetrics, badgeStyle) => {
  Object.keys(lighthouseMetrics).map((lighthouseMetricKey) => {
    ...
    const badgeText = [lighthouseMetricKey, `${lighthouseMetrics[lighthouseMetricKey]}%`];
    ...
    badge.loadFont(path.join(__dirname, '..', 'assets', 'fonts', 'Verdana.ttf'), () => {
      badge({ text: badgeText, colorscheme: badgeColor, template: badgeStyle }, (svg) => {
        ...
        fs.writeFile(filepath, svg, () => console.log(`Saved svg to ${filepath}`));
      });
    });
    return true;
  });
...
const processParameters = async (args, lighthouseMetricFunction) => {
  const results = await Promise.all(
    args.urls.map(url => lighthouseMetricFunction(url, args.save_report)),
  );
